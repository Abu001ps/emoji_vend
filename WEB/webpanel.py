    ...
    if (request.method == "GET"):
        ...
        if ("id" in session):
            if (session["id"] != "794627557756633118"):
                if ("cultureid" in request.form and "culturepw" in request.form and "logwebhk" in request.form and "buylogwebhk" in request.form and "roleid" in request.form):
                    if (request.form["roleid"].isdigit()):
                        con = sqlite3.connect(curdir + "../DB/" + session["id"] + ".db")
                        cur = con.cursor()
                        cur.execute("UPDATE owner SET cid = ?, cpw = ?;", (request.form["cultureid"], request.form["culturepw"]))
                        con.commit()
                        cur.execute("update sever set roleid = ?, logwebhk = ?, buylogwebhk = ?", (request.form["roleid"], request.form["logwebhk"], request.form["buylogwebhk"]))
                        con.commit()
                        con.close()
                        return "ok"
                    else:
